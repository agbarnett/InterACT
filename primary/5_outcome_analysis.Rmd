...
trial_dates = mutate(trial_dates, study_label = deidentify_hospital(study_label)) %>% # blind, added AGB
  arrange(study_label)
...
# From 3_define_outcomes.R
load('analysis_ready/trial_outcomes_data.rda')

## AGB, blind hospital
#
cristal_spict_admission_info = mutate(cristal_spict_admission_info,
                  fclty_name = deidentify_hospital_long(fclty_name),
                  fclty_name_ordered = factor(fclty_name, levels=c('X','Y','Z'), ordered =TRUE))
#
cristal_spict_admission_info = cristal_spict_admission_info %>%
  mutate(Hospital = str_remove_all(study_id,'_.*'),
         Hospital = deidentify_hospital(Hospital)) # blind, added AGB 

## intention to treat or not
## AGB: Change one team that did not receive intervention to control only (usual care at all times)
itt = TRUE # intention to treat
if(itt == FALSE){
cristal_spict_admission_info = mutate(cristal_spict_admission_info,
          study_period = as.character(study_period),            
          study_period = ifelse(clinical_team_fclty=='4', 'Usual care', study_period),
          study_period = as.factor(study_period))
}
```
...
Results presented in this report are limited to patients classified as ``at-risk'' due to a positive SPICT or CRiSTAL screening. Clinical teams and hospitals are de-identified.
...
# draws flow chart
  ...
  included <- boxGrob(glue("Total at-risk admissions analysed: n = {final_n}",
                           " - Hospital X: n = {n_x}", # blinded
                           " - Hospital Y: n = {n_y}",
                           " - Hospital Z: n = {n_z}",
                           final_n = format(n2,big.mark=','),
                           n_z = format(filter(n_cluster_teams,Hospital=='Z')[['n_atrisk']],big.mark=','),
                           n_x = format(filter(n_cluster_teams,Hospital=='X')[['n_atrisk']],big.mark=','),
                           n_y = format(filter(n_cluster_teams,Hospital=='Y')[['n_atrisk']],big.mark=','),
                           .sep = "\n"),
                      just = "left")
  #
...
```{r}
#
flextable(trial_info) %>% 
          merge_v(j='Hospital')%>%
         theme_box() %>% 
         fontsize(size=9,part='all') %>%
         width( width = c(0.7, 1.75, 1.4, 1.4, 0.6, 0.6)) %>% 
         set_caption(caption='Trial dates by hospital')
```
...
n_atrisk_byphase = cristal_spict_admission_info %>% 
 group_by(Hospital,study_period) %>% summarise(n_atrisk = n(),.groups='drop') %>% spread(study_period,n_atrisk) %>% select(Hospital,`Usual care`,`Intervention exposure`) %>%
  mutate(`Total at-risk`=`Usual care`+`Intervention exposure`) %>% 
...
cristal_spict_admission_info = cristal_spict_admission_info %>% 
  mutate_at('study_period',~relevel(factor(.),ref='Usual care')) 
...
```{r baseline}
## Add back in SPICT and CRISTAL for table
load('../data/FullData.RData')
scores = select(baseline, participant_id, cristal_score, spict_score) %>%
  rename(study_id = participant_id)
for_table = left_join(cristal_spict_admission_info, scores, by='study_id') %>%
  mutate(at_risk_date = as.Date(cristal_spict_date_time)) %>%
  rename('age' = 'age_on_admission')

## Vector of all variables to summarise (in order)
myVars <- c("at_risk","at_risk_date","sex","age","cristal_score","spict_score")
## Vector of categorical variables
catVars <- c('at_risk',"sex")
medians = c("age","cristal_score","spict_score",'at_risk_date')
...
print(tab1, 
      ...
      contDigits = 1, 
      nonnormal = medians, # give median not mean/SD
      explain = TRUE, 
      showAllLevels = TRUE,
...
Summary stats for at-risk dates:

```{r}
dates = group_by(for_table, study_period) %>%
  summarise(median = median(at_risk_date),
            q1 = quantile(at_risk_date, 0.25, type=1),
            q3 = quantile(at_risk_date, 0.75, type=1),
            min = min(at_risk_date),
            max = max(at_risk_date)) %>%
  ungroup()
flextable(dates) %>%
  theme_box() %>%
  autofit()
...
### Relevant text from the protocol (Outcome 1)
...
## AGB: Change one team that did not receive intervention to control only (usual care at all times)
if(itt == FALSE){
# with(dat_analysis, table(study_period, clinical_team_fclty)) # check
dat_analysis = mutate(dat_analysis,
          study_period = as.character(study_period),            
          study_period = ifelse(clinical_team_fclty=='4', 'Usual care', study_period),
          study_period = as.factor(study_period)) # transform back
# with(dat_analysis, table(study_period, clinical_team_fclty)) # check
}

dat_analysis = dat_analysis %>% 
  mutate(Hospital = str_remove_all(study_id,'_.*'),
         Hospital = deidentify_hospital(Hospital), # blind
         study_period = case_when(intervention==0~'Usual care',intervention==1~'Intervention phase') %>% factor(.,level=c('Usual care','Intervention phase')))
ftab_icu_n = dat_analysis %>% group_by(Hospital,study_period) %>% 
  summarise(n = sum(status_icu),p = roundz(100*mean(status_icu),1),.groups = 'drop') %>% 
  mutate(cell=paste0(n,' (',p,')')) %>% 
  select(-n,-p) %>% 
  spread(study_period,cell) 
...
ad = dat_analysis %>% 
  ...
  mutate(cell=paste0(n,' (',p,')')) %>% 
  select(-n,-p) %>%
  add_column('Hospital'='All',.before=1) %>% 
  spread(study_period, cell) 

ftab_icu_n = bind_rows(ftab_icu_n, ad)
  
ftab_icu_n = flextable(ftab_icu_n) %>%
  theme_box() %>% 
  autofit() %>%  
  fontsize(size=9, part='all') 
...
g1 = dat_analysis %>% ggplot(aes(age_scaled*5+85,status_icu,group=Hospital,colour=Hospital))+
  stat_ecdf(size=1)+
  facet_grid(~study_period) + 
  scale_colour_manual('Hospital', values=cbPalette)+
  scale_x_continuous('Age at hospital admission, years',breaks=seq(75,120,5))+
  scale_y_continuous('Cumulative proportion of 1+ ICU admissions\nin dataset',breaks=seq(0,1,0.1))+
  g.theme

g1 
```

In the next figure, we show the weekly numbers and percentages of patients with 1+ ICU admissions over time. Weekly totals are based on week of study enrollment/identification as CriSTAL/SPICT positive. There was some evidence of differences in weekly percentages by hospital, however, total patient numbers per week were similar. Weekly percentages were highest at Y, however, this was driven by smaller weekly numbers of at-risk hospital admissions, compared with X and Z.
...
g2 = dat_analysis %>% 
  group_by(Hospital,study_week) %>% 
  summarise('1+ ICU admissions (%)' = 100*mean(status_icu),
                                                             '1+ ICU admissions (n)' = sum(status_icu),
                                                             n = n(),
                                                             .groups='drop') %>%
    right_join(tibble(study_week=0:53),by='study_week') %>% 
  gather(variable,value,-Hospital,-study_week) %>%
...
### Mixed model analysis (Outcome 1)

#### intention to treat analysis (Outcome 1)
...
Marginal effects for patient age, study week are shown in the figure below (Panels A and B, respectively). Estimated random effects for each team (within hospital) are shown in Panel C.
...
icu.re = icu.re %>% 
  left_join(ad,by=c('grp'='clinical_team_fclty')) %>% 
  mutate_at('grp',~as.numeric(as.character(.))) 
...
g.icu_re = ggplot(icu.re, aes(y=grp,x=condval,colour=Hospital)) +
  ...
  scale_y_continuous('Clinical team',breaks=1:14)+
  g.theme
...
#### Checking for influential observations (Outcome 1)
...
#### Sensitivity analysis (Outcome 1)
...
ftab.icu_sens = inner_join(ftab.icu_sens,cis,by='term') %>%
  mutate(Parameter = case_when(
           ...
           term=='age_scaled' ~ 'Patient age (+5 years; mean = 85 years)',
...
#### Overall event probabilities (Outcome 1)


The next table shows the estimated event probabilities of ICU admissions by study phase. Results are reported for intention to treat and sensitivity analyses. 
...
bind_rows(list("intention to treat"=ftab.icu_effect,"Sensitivity"=ad),.id="Analysis") %>% flextable() %>% 
...
#### Event probabilities (Outcome 1)

The results show the difference in the estimated probability of ICU admission between the usual care and intervention phases.


#### Event probabilities by hospital (Outcome 1)

The results show the difference in the estimated probability of ICU admission between the usual care and intervention phases.

```{r}
load('bootstrap output/icu.rda') # from 5_bootstrapped_outputs.R

diff_icu %>% 
  flextable() %>% 
  merge_v(j=c("Analysis")) %>% 
  theme_box() %>% 
  fontsize(size=9,part='all') %>% 
  colformat_double(j=3:5, digits=3) %>%
  width(width=c(1,1.5,1,1,1.5)) %>% 
  set_caption('Probabilities')
```

\newpage

## Analysis changing one clinical team (Outcome 1)

One clinical team did not fully receive the intervention because they did not get the emails. Hence we use a sensitivity analysis to assess how this impacted the results. In this analysis the clinical team remains in the usual care phase and does not move over to the intervention.

```{r}
# alter data so that team stays in usual care
dat_analysis_pp = mutate(dat_analysis, 
            intervention = ifelse(clinical_team_fclty=='4', 0, intervention))
icu.mod_pp <- glmer(status_icu ~ intervention + study_week + age_scaled + sex + (1|clinical_team_fclty), family=binomial('logit'), data=dat_analysis_pp)

#
ftab.icu_glmer_pp = summary(icu.mod_pp)$coefficients %>% 
  as.data.frame() %>% 
  rownames_to_column(var='Parameter') %>% 
  select(Parameter,Estimate,`Std. Error`) %>% 
  mutate_at('Estimate',~exp(.)) 
#
icu_glmer_ci_pp = exp(confint.merMod(icu.mod_pp, method="Wald")) %>% 
  as.data.frame() %>% 
  rownames_to_column(var="Parameter")
#
ftab.icu_glmer_pp = ftab.icu_glmer_pp %>% left_join(icu_glmer_ci_pp, by="Parameter")

ftab.icu_glmer_pp = ftab.icu_glmer_pp %>% 
  mutate('Odds ratio (95% CI)' = paste0(roundz(Estimate,2),' (',roundz(`2.5 %`,2),' to ',roundz(`97.5 %`,2),')')) %>%
  select(Parameter,`Odds ratio (95% CI)`) %>% 
  mutate_at("Parameter", ~ case_when(
    .=='(Intercept)' ~ 'Intercept',
...
#               
ftab.icu_glmer_pp = filter(ftab.icu_glmer_pp, Parameter!="Intercept") %>% 
  flextable() %>% 
  theme_box() %>% 
  fontsize(size=9,part='all') %>% 
  width(width=c(3.2,1.5))

ftab.icu_glmer_pp %>% set_caption('Binomial mixed effects regression: 1+ ICU admissions')
...
#### Relevant text from the protocol (Outcome 2)
...
#### CI feedback during data processing (Outcome 2)
...
ftab_los = ftab_los %>% 
  ...
  merge_v(j=1) %>%
  fontsize(size=9,part='all') %>% 
  width(width=c(0.75,1.5,rep(1.75,3)))
...
#### Separate plots for death and discharge

The plots above rather squash the effect of death, hence we also include separate plots.
...
g.cif_los_a <- lapply(g,function(x) x$data) %>% bind_rows(.id='Hospital') %>%
  filter(event=='Died in hospital') %>%
  mutate_at("name",~str_wrap(.,30)) %>%
  ggplot(aes(x=time,y=est,ymin=est-1.96*std,ymax=est+1.96*std,group=name,colour=group,fill=group))+
  geom_ribbon(alpha=.2)+
  geom_path(size=1.25)+
  facet_grid(~Hospital)+
  ...
  scale_x_continuous('Days since first recorded as high-risk CriSTAL or SPICT-positive',breaks=seq(0,90,10))+
  expand_limits(y=1)+
  scale_y_continuous('Cumulative probability', breaks=seq(0, 0.15, 0.05), limits=c(0, 0.15))+
  g.theme
...
```{r}
g.cif_los_b <- lapply(g,function(x) x$data) %>% bind_rows(.id='Hospital') %>%
  ...
  scale_x_continuous('Days since first recorded as high-risk CriSTAL or SPICT-positive',breaks=seq(0,90,10))+
  expand_limits(y=1)+
  scale_y_continuous('Cumulative probability', breaks=seq(0, 1, 0.2), limits=c(0, 1))+
  g.theme

g.cif_los_b
```

### Time-to-event modelling (Outcome 2)

#### intention to treat analysis (Outcome 2)
...
#### Model assessment (Outcome 2)

##### Baseline hazards (Outcome 2)

Here we plot the baseline hazard in each clinical team. This is useful for checking the differences between hospitals (numbered facets). 
...
##### Checking for influential observations (Outcome 2)
...
Model assessment results indicated that non-proportionality due to age was strongest for GCUH (next figure).
...
#### Sensitivity analysis (Outcome 2)
...
#### Overall absolute differences (Outcome 2)
...
```{r}
#includes all hospitals and by site, from ?.R
load('bootstrap output/length of stay.rda') 

risk_difference_los %>% 
  flextable() %>% 
  merge_v(j=c("Analysis","Hospital outcome")) %>% 
  theme_box() %>% fontsize(size=9,part='all') %>% 
  width(width=c(1,1.5,1,1,1.5)) %>% 
  set_caption('90-day probabilities of death and discharge')


```

The next table shows estimated differences in length of stay for intention to treat and sensitivity analyses. For the sensitivity analysis, estimates were obtained by setting calendar time to the study end date (6 June 2021).
...
#### Absolute differences by hospital (Outcome 2)
...
risk_difference_los_h %>% 
  flextable() %>% 
  merge_v(j=c("Analysis","Hospital outcome")) %>% 
  theme_box() %>% fontsize(size=9,part='all') %>% 
  width(width=c(1,1.5,1,1,1,1.5)) %>% 
  set_caption('90-day probabilities of death and discharge')
...
```{r}
rmst_difference_los_h %>% flextable() %>% merge_v(j="Analysis") %>% theme_box() %>% fontsize(size=9,part='all') %>% width(width=c(1,1.5,1,1,1,2)) %>% set_caption('Estimated hospital length of stay for usual care and Intervention phases')
```


## Analysis changing one clinical team (Outcome 2)
...
dat_analysis_pp = mutate(dat_analysis, 
            intervention = ifelse(clinical_team_fclty=='4', 0, intervention))
# run model
mod.los_pp <- coxph(Surv(length_of_stay_days_c_sens, status_los_sens) ~ intervention + age_scaled + sex +  strata(clinical_team_fclty), data=dat_analysis_pp, id=study_id)

# make table
ftab.los_coxph_pp = summary(mod.los_pp)$conf.int %>% 
  as.data.frame() %>% 
  rownames_to_column(var='term') %>%
  separate(term,into=c('term','event'),sep=':') %>%
  mutate(term = str_remove_all(term,'_1'),
         'Hospital outcome'=factor(event,levels=2:3,labels = mod.los_pp$states[2:3]),
         Parameter = case_when(
           term=='intervention' ~ 'Intervention phase',
           term=='age_scaled' ~ 'Patient age (+5 years; mean = 85 years)',
           term=='sexMale' ~ 'Sex = Male'),
...
ftab.los_coxph_pp %>% 
  ...
  theme_box() %>% 
  fontsize(size=9,part='all') %>% 
  width(width=c(2.5,1.5,1.5)) %>% 
  set_caption('Cox regression output for hospital length of stay, all study hospitals')
...
### Relevent text from the protocol (Outcome 3)
...
### Descriptive summary (Outcome 3)


```{r}
ftab_readmit = filter(dat_analysis,discharge_outcome=='Discharged alive') %>% 
  group_by(Hospital,study_period) %>% 
  summarise(n = sum(status_readmit=="Readmitted to public hospital"),
            p = roundz(100*mean(status_readmit=="Readmitted to public hospital"),1),.groups='drop') %>% 
  mutate('Re-admissions to hospital'=paste0(n,' (',p,')')) %>% 
  select(-n,-p)

ad = filter(dat_analysis,discharge_outcome=='Discharged alive') %>% 
  group_by(study_period) %>% 
  summarise(n=sum(status_readmit=="Readmitted to public hospital"),
            p=roundz(100*mean(status_readmit=="Readmitted to public hospital"),1),.groups='drop') %>%
  mutate('Re-admissions to hospital'=paste0(n,' (',p,')')) %>% 
...
ftab_readmit = ftab_readmit %>% flextable() %>% 
  merge_v(j = 'Hospital') %>%
  theme_box() %>% fontsize(size=9,part='all') %>% width(width=c(0.75,2.25,1.75,1.75))


ftab_readmit %>% set_caption('Descriptive statistics -  number of readmissions, n (%), and days to readmission, up to 12 weeks following hospital discharge, Median (Q1 to Q3)')
...
### Time-to-event modelling (Outcome 3)

#### intention to treat analysis (Outcome 3)
...
#### Model assessment (Outcome 3)

##### Baseline hazards (Outcome 3)

The next figure shows estimated baseline hazards by clinical team. The numbers in round brackets in the legend are the number of patients.
...
##### Checking for influential observations (Outcome 3)
...
dfbeta_readmit = influence_readmit$data %>%
  mutate(Parameter = case_when(
           ...
           covariate=='age_scaled' ~ 'Patient age (+5 years; mean = 85 years)',
...
#### Adjust for season (Outcome 3)

```{r}
# added by AGB
mod.readmit.season <- coxph(Surv(discharge_to_readmit_days_c, status_readmit=="Readmitted to public hospital") ~ intervention + age_scaled + sex + strata(clinical_team_fclty) + sinw + cosw, 
                     subset=(discharge_outcome=='Discharged alive'),
                     data=dat_analysis)

zph.readmit.season = cox.zph(mod.readmit.season, transform='identity')

ftab.readmit_coxph_season = summary(mod.readmit.season)$conf.int %>% 
  as.data.frame() %>% 
  rownames_to_column(var='term') %>%
  mutate(Parameter = case_when(
           term=='intervention' ~ 'Intervention phase',
...
ftab.readmit_coxph_season %>% 
  flextable() %>% 
  theme_box() %>% 
  fontsize(size=9,part='all') %>% 
  width(width=c(2.5,1.5)) %>% set_caption('Cox regression output for time to readmission, all study hospitals, adjusted for season')



```

#### Results by hospital (Outcome 3)
...
Model assessment plots are shown in the next figure. Results indicate that evidence of non-proportionality for Intervention phase is strongest at Hospital Z.
...
#### Sensitivity analysis (Outcome 3)
...
##### Time-dependent effect for Intervention phase (Outcome 3)
...
legend(0, -.6, legend=c("Estimated intervention effect from residuals (95% CI)", "Binary fit assumed (intention to treat)","Time-dependent effect assumed (linear)"),col=cbPalette[1:3], lwd=2, cex=0.8)

```

##### Seasonality (Outcome 3)
...
#### Overall absolute differences (Outcome 3)
...
risk_difference_readmit %>% 
  flextable() %>% 
  merge_v(j="Analysis") %>% 
  theme_box() %>% 
  fontsize(size=9,part='all') %>% 
  width(width=c(2,1,1,1.5)) %>% 
  set_caption('90-day probabilities of readmission to a public hospital')
...
#### Overall absolute differences by hospital (Outcome 3)

The final tables show the same estimates by study phase and hospital.

```{r}
# includes all hospitals and by site, from 5_bootstrapped_outputs.R
load('bootstrap output/hospital readmission by hospital.rda') 

risk_difference_readmit_h = mutate(risk_difference_readmit_h, 
                                   Hospital = as.character(Hospital))


risk_difference_readmit_h %>% 
  flextable() %>% 
  merge_v(j="Analysis") %>% 
  theme_box() %>% 
  fontsize(size=9,part='all') %>% 
  width(width=c(2,1,1,1,1.5)) %>% 
  set_caption('90-day probabilities of readmission to a public hospital')
...
```{r}
rmst_difference_readmit_h %>% flextable() %>% theme_box() %>% fontsize(size=9,part='all') %>% width(width=c(1,1,1,1,2)) %>% set_caption('Estimated time to readmission for usual care and Intervention phases')
```

### Analysis changing one clinical team (Outcome 3)
...
mod.readmit_pp <- coxph(Surv(discharge_to_readmit_days_c, status_readmit=="Readmitted to public hospital") ~ intervention + age_scaled + sex + strata(clinical_team_fclty), 
                     subset=(discharge_outcome=='Discharged alive'),
                     data=dat_analysis_pp)

ftab.readmit_coxph_pp = summary(mod.readmit_pp)$conf.int %>% as.data.frame() %>% rownames_to_column(var='term') %>%
  mutate(Parameter = case_when(
           term=='intervention' ~ 'Intervention phase',
           term=='age_scaled' ~ 'Patient age (+5 years; mean = 85 years)',
           term=='sexMale' ~ 'Sex = Male'),
...
ftab.readmit_coxph_pp %>% 
  flextable() %>% 
  theme_box() %>% 
  fontsize(size=9,part='all') %>% 
  width(width=c(2.5,1.5)) %>% 
  set_caption('Cox regression output for time to readmission, all study hospitals, per protocol')
```
...
### Relevant text from the protocol (Outcome 7)
...
dat_analysis = mutate(dat_analysis,
                      ...
                      screening_to_met_days_cens = pmin(90,screening_to_met_days)) %>%
...
mod.cif_met <- lapply(hospitals,function(x)
  with(filter(dat_analysis,Hospital==x),cuminc(screening_to_met_days_cens,status_met_sens,group=intervention,strata=as.character(clinical_team_fclty),cencode='censor')))
...
# save all cumulative plots for using in 6_cumulative_plot.R
save(g.cif_met, g.cif_los_a, g.cif_los_b, g.cif_readmit_a, g.cif_readmit_b, file='results/cumulative_plots.RData')
```

### Time-to-event modelling (Outcome 7)

#### intention to treat analysis (Outcome 7)
...
mod.met <- coxph(Surv(screening_to_met_days_cens, status_met_sens=="MET call") ~ intervention + age_scaled + sex +  strata(clinical_team_fclty), data=dat_analysis)
...
ftab.met_coxph = summary(mod.met)$conf.int %>% as.data.frame() %>% rownames_to_column(var='term') %>%
  mutate(Parameter = case_when(
           term=='intervention' ~ 'Intervention phase',
           term=='age_scaled' ~ 'Age at admission (+5 years; mean = 85 years)',
           term=='sexMale' ~ 'Sex = Male'),
...
#### Model assessment (Outcome 7)

##### Baseline hazards (Outcome 7)
...
#### Checking for influential observations (Outcome 7)
...
#### Results by hospital (Outcome 7)

The results below are from separate Cox models in each hospital. Estimated hazard ratios for Intervention phase vary between the hospitals.

```{r}
mod.met_h <- lapply(hospitals,function(x) coxph(Surv(screening_to_met_days_cens, status_met_sens=="MET call") ~ intervention + age_scaled + sex +  strata(clinical_team_fclty), data=dat_analysis, subset=(Hospital==x), id=study_id))
...
#### Sensitivity analysis (Outcome 7)
...
```{r}
#dat_analysis = mutate(dat_analysis,admit_month=factor(month(admit_date),levels=1:12))
mod.met_sens <- coxph(Surv(screening_to_met_days_cens, status_met_sens=="MET call") ~ intervention + age_scaled + sex + sinw + cosw + strata(clinical_team_fclty), data=dat_analysis)
...
#### Absolute differences by hospital (Outcome 7)
...
risk_difference_met_h %>% flextable() %>% merge_v(j="Analysis") %>% theme_box() %>% fontsize(size=9,part='all') %>% width(width=c(2,1,1,1,1.5)) %>% set_caption('90-day probabilities of MET call')
...
```{r}
rmst_difference_met_h %>% flextable() %>% theme_box() %>% fontsize(size=9,part='all') %>% width(width=c(1,1,1,1,2)) %>% set_caption('Estimated time to first MET call for usual care and Intervention phases')
```

### Analysis changing one clinical team (Outcome 7)
...
dat_analysis_pp = mutate(dat_analysis, 
            intervention = ifelse(clinical_team_fclty=='4', 0, intervention))
## 
mod.met_pp <- coxph(Surv(screening_to_met_days_cens, status_met_sens=="MET call") ~ intervention + age_scaled + sex +  strata(clinical_team_fclty), data=dat_analysis_pp)

ftab.met_coxph_pp = summary(mod.met_pp)$conf.int %>% 
  as.data.frame() %>% 
  rownames_to_column(var='term') %>%
  mutate(Parameter = case_when(
           term=='intervention' ~ 'Intervention phase',
...
ftab.met_coxph_pp %>% 
  ...
  theme_box() %>% 
  fontsize(size=9,part='all') %>% 
  width(width=c(2.5,1.5)) %>% set_caption('Cox regression output for time to first MET call, all study hospitals, per protocol')

```

# Summary plot (all outcomes)
...
ests_met_coxph_h = lapply(1:3, function(x)
  ...
  mutate(type = 'Medical Emergency Team',
         hospital = as.numeric(str_remove_all(term, '[^0-9]')), #? change now
...
## 
facet_labels = c('Time to discharge',
...
ests_los_coxph_h = lapply(1:3, function(x)
  ...
  filter(str_detect(term, 'intervention')) %>%
  clean_names() %>%
  mutate(type = ifelse(str_detect(term,'1:3'), 'Time to in-hospital death', 'Time to discharge'),
         hospital = rep(1:3,2))
...
to_plot = bind_rows(ests_met_coxph_h,
                    ...
                    ests_los_coxph_h) %>%
         mutate(typen = case_when( # for ordering
           type == 'Time to discharge' ~ 1, 
           type == 'Time to in-hospital death' ~ 2, 
           type == 'Medical Emergency Team' ~ 3, 
           type == 'Re-admission' ~ 4 
         ),
         typen = factor(typen, levels=1:4, labels=facet_labels),
         Hospital = case_when( # blind hospital using X, Y, Z
           hospital == 1 ~ 'Z',
           hospital == 2 ~ 'Y',
           hospital == 3 ~ 'X'
         ))
...
## plot
# labels by dotted line
labels = data.frame(Hospital = 3.2, exp_coef = c(1,1), 
                    lower_95 = 1, upper_95 = 1,
                    adj = c(1.05, -0.05),
                    typen = 'Time to re-admission', # just in this panel
                    label = c("Decrease in\nintervention phase",
                              "Increase in\nintervention phase"))
...
gplot = ggplot(to_plot, aes(x=Hospital, y = exp_coef, ymin=lower_95, ymax=upper_95))+
  ...
  geom_text(data = labels, aes(x=Hospital, y=exp_coef, label=label, adj=adj), col='grey44', size=2)+
  geom_point(size=2.5, col='dodgerblue')+
  geom_errorbar(width=0, col='dodgerblue', size=1.05)+
  scale_x_discrete(breaks=c('Z','Y','X'), expand = c(0.07, 0.20))+
  xlab('Hospital')+
  ylab('Hazard ratio')+
  coord_flip()+
  theme_bw()+
  theme(panel.grid.minor = element_blank())+
  facet_wrap(~typen, scales='free')
gplot
# export to figure
...
The information below shows the version of R and the packages used in the above analysis.
