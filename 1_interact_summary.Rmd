...
frailty_threshold = 5
frailty_threshold2 = 7 # second threshold added in May 2020
...
cristal = select(baseline, "participant_id", 'hospital', cristal.vars) %>%
  mutate(
    #age = ifelse(age>=75, 'Yes', 'No'), # convert to binary , no longer using age in cristal
         cristal_cfs_score1 = ifelse(cristal_cfs_score >= frailty_threshold, 'Yes', 'No'), # frailty score over 5,
         cristal_cfs_score2 = ifelse(cristal_cfs_score >= frailty_threshold2, 'Yes', 'No') # frailty score over 7,
         ) %>% 
...
update = mutate(baseline, 
                ...
                `CriSTAL positive` = as.numeric(cristal_score >= cristal_threshold),
                `CriSTAL positive` = factor(`CriSTAL positive`, levels=0:1, labels=c('No','Yes')),
                `CriSTAL positive new` = as.numeric(cristal_score_new >= cristal_threshold),
