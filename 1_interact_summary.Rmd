...
for.missing = select(palliative_care_referral, -participant_id, -hospital, # remove variables that cannot be missing
                     -starts_with('start_date_time'), -starts_with('end_date_time'), # not that important
                     -redcap_version,
                     -datediff_pallcare) # remove conditional variables
naniar::gg_miss_var(for.missing, show_pct = FALSE)
```
...
for.missing = select(care_directive, -participant_id, -hospital, # remove variables that cannot be missing
                     -starts_with('start_date_time'), -starts_with('end_date_time'), # not that important
                     -redcap_version,
                     -datediff_care_directive, -tracker, -care_directive_other) # remove conditional variables
naniar::gg_miss_var(for.missing, show_pct = FALSE)
```
...
cplot = ggplot(list_resp, aes(x = spicts)) +
    ...
    g.theme+
  theme(plot.margin = margin(t = 0, r = 0, b = 0, l = 80, unit = "pt"))
cplot
```
...
Here we examine if adding a second point to the total CriSTAL score for `r thresholds$frailty_threshold2`+ on the clinical frailty score makes a difference to the number of CriSTAL positive patients. The lower threshold is at a clinical frailty score of `r thresholds$frailty_threshold`.

```{r, results='asis'}
update = mutate(baseline, 
                cristal_score_new = cristal_score + as.numeric(cristal_cfs_score >= thresholds$frailty_threshold2), # add one more to score for high frailty
                `CriSTAL positive` = as.numeric(cristal_score >= thresholds$cristal_threshold),
                `CriSTAL positive` = factor(`CriSTAL positive`, levels=0:1, labels=c('No','Yes')),
                `CriSTAL positive new` = as.numeric(cristal_score_new >= thresholds$cristal_threshold),
...
with(clinicianled_review, descr(-datediff_clinician_review, stats=c('N.Valid','min','Q1','med','Q3','max'), round.digits=0, cumul = FALSE, report.nas =FALSE))
```

#### Histogram of difference

```{r}
hplot = ggplot(data=clinicianled_review, aes(x=-datediff_clinician_review))+
...
# Care directive measure

### Has there been a review of any care directives?

```{r, results='asis'}
with(care_directive, freq(change_5, round.digits=0, cumul = FALSE, report.nas =FALSE))
```

### Difference between screening date and initiation of care directive measure

#### Table of difference

```{r, results='asis'}
# note, convert time to negative
with(care_directive, descr(-datediff_care_directive, stats=c('N.Valid','min','Q1','med','Q3','max'), round.digits=0, cumul = FALSE, report.nas =FALSE))
```

#### Histogram of difference

```{r}
hplot = ggplot(data=care_directive, aes(x=-datediff_care_directive))+
...
### Have any of the following outcomes occurred?
...
```{r, results='asis'}
with(palliative_care_referral, freq(pall_care, round.digits=0, cumul = FALSE, report.nas =FALSE))
```
...
```{r, results='asis'}
# note, convert time to negative
with(palliative_care_referral, descr(-datediff_pallcare, stats=c('N.Valid','min','Q1','med','Q3','max'), round.digits=0, cumul = FALSE, report.nas =FALSE))
```

#### Histogram of difference

```{r}
hplot = ggplot(data=palliative_care_referral, aes(x=-datediff_pallcare))+
