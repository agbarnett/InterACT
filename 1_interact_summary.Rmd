...
if(scramble==TRUE){
  set.seed(123456)
  data$int_time = sample(data$int_time, size=nrow(data), replace=FALSE) # scramble intervention time
}
```
...
### Examining patients with no follow-up forms

Here we examine those patients who were at risk but did not have their follow-up forms completed.

```{r}
# prepare data used below
to_table = filter(baseline, at_risk=='At risk') %>%
  select(participant_id, hospital, complete_456, earliest_form, spict_score, cristal_score, contains('date')) %>%
  mutate(#admission_datetime = as.numeric(admission_datetime),
         diff = (admission_datetime - earliest_form)/(60*60*24), # in days
         admission_datetime = as.POSIXct(admission_datetime, origin='1970-01-01'),
         earliest_form = as.POSIXct(earliest_form, origin='1970-01-01')) %>%
  left_join(complete, by=c('participant_id','hospital')) # add form completion
```

##### Was it because they were already discharged?

```{r}
# were they discharged before the forms could be completed?
dplot = ggplot(data=to_table, aes(x=complete_456, y=diff))+
  geom_boxplot()+
  xlab('Completed Follow-up')+
  ylab('Days between forms and admission')+
  theme_bw()
dplot
```

The lack of difference in days between the groups indicates that this is not a major factor.

##### Was it because they are ongoing cases?

```{r}
# 
dplot = ggplot(data=to_table, aes(x=complete_456, y=admission_date))+
  geom_boxplot()+
  xlab('Completed Follow-up')+
  ylab('Admission date')+
  theme_bw()
...
The lack of difference in days between the groups indicates that admission date is not a major factor.


##### Was it because their baseline screening forms were not complete?

```{r, results='asis'}
# 
with(to_table, ctable(discharge_forms, complete_456, prop='r', useNA='always', round.digits=0, dnn=c('Dischage','')))
```

The table rows are whether the baseline screening forms are complete, and the columns are whether the follow-up forms are complete. For those with baseline screening not completed, almost all do not have their follow-up forms completed. 

#### Do they differ in the SPICT and CriSTAL scores?

```{r}
to_plot = select(to_table, participant_id, complete_456, spict_score, cristal_score) %>%
  gather(`spict_score`, `cristal_score`, key='outcome', value='score') %>%
  mutate(outcome = ifelse(str_detect(string=outcome, pattern='spict'), 'SPICT','CriSTAL'))
bplot = ggplot(to_plot, aes(x=complete_456, y=score))+
  geom_boxplot()+
  xlab('Follow-up complete')+
  ylab('Score')+
  facet_wrap(~outcome, scales='free_y')+
  theme_bw()
bplot
# output
jpeg('figures/scores_followup_complete.jpg', width=4.5, height=3.5, res=300, units='in', quality = 100)
print(bplot)
invisible(dev.off())
```

It does appear that for patients with follow-up complete, the SPICT score was clearly higher. Hence those patients we missed were likely not as sick.
...
* The earliest date/time from the four screening forms: hospital admissions, clinical criteria and functional status and comorbidities. This is an estimate of when screening started.
...
to_table = left_join(baseline, for_merge, by='participant_id') %>%
  mutate(
    diff_start = (as.numeric(earliest_form) - as.numeric(admission_datetime))/(60*60*24), # in days from earliest date
...
### Secondary outcome: Time to first care directive measure

Here we use a survival model to examine the time to outcome.

The plot below shows the cumulative incidence for the first care directive measure by hospital.

```{r}
library(cmprsk)
library(survminer)
# set up data
for_model = filter(clinicianled_review, time_change >=0) # exclude negative times
excluded = nrow(clinicianled_review) - nrow(for_model)
# cumulative incidence plot
# to add treatment group, hospital as strata?
cum_inc = with(clinicianled_review, cuminc(ftime=time_change, fstatus=change_var=='Yes', group=hospital))
# model - do not use Cox, protocol says otherwise
#model = coxph(Surv(time = time_change, event = change_var=='Yes') ~ hospital, data=clinicianled_review)
#ests = tidy(model)
# to here, need to create regression model
...
There were `r excluded` patients excluded as their times were negative
