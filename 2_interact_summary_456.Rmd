...
The time-to-event analyses used the time to the first "Yes" outcome (e.g., time to first clinical-led care review). For patients with only "No" outcomes, the time was censored at the date/time of their last "No" outcome. Discharge date was used to censor all patients in the GCUH and RBWH hospitals. For TPCH, we could only used discharge as a censoring time in a subset of 512 patients who were retrospectively audited. For other TPCH patients we ended their survival time at their last completed audit.
...
to_plot_care_review = mutate(plot_data,
                 hosp_ordered = factor(hospital, levels=c('TPCH','RBWH','GCUH')), # to show in intervention order
                 z = qnorm(0.975),
                 lower  = est - (z*sqrt(var)),
                 upper  = est + (z*sqrt(var)),
                 group = ifelse(group=='Usual', 'Usual care', group))
# redo plot for greater control
alt_plot = ggplot(to_plot_care_review, aes(x=time, y=est, ymin=lower, ymax=upper, col=group)) +
...
for_model_care_review = mutate(for_model_care_review,
  event = ifelse(event=='Prior', 'Yes', event))
# Cox model stratified on team within hospital
cox_model_care_review = coxph(Surv(time = time, event = event== 'Yes') ~ int_time_n + I(age/5) + pt_sex +  cristal_score + spict_score + strata(team), data = for_model_care_review)
# number missing:
...
for (hosp in c("GCUH","TPCH",'RBWH')){
  ## Cox model stratified on team within hospital
  cox_model_care_review_hosp = coxph(Surv(time = time, event = event== 'Yes') ~ int_time_n + I(age/5) + pt_sex +  cristal_score + spict_score + strata(team), data = filter(for_model_care_review, hospital==hosp))
  #
...
# make the table of estimates
  all_ests_cox_care_review = filter(all_ests_cox_care_review, !is.na(estimate)) %>%
  ...
  pivot_wider(values_from=cell, names_from=hospital) %>%
    select(Variable, TPCH, RBWH, GCUH) # In intervention order
# table
ft = flextable(all_ests_cox_care_review) %>%
  theme_box() %>%
  autofit()
ft
```
...
if(exists == FALSE){
  risk_difference_care_review = risk_diff_crr(
    ...
    B = n_boot,
    pred_time = pred_time)
  save(risk_difference_care_review, file = 'results/clinical_led_absolute_results.RData')
}
if(exists == TRUE){load('results/clinical_led_absolute_results.RData')}
...
if(exists == FALSE){
  ...
  save(all_diffs_care_review, file = 'results/clinical_led_by_hospital_results.RData')
}
if(exists == TRUE){load('results/clinical_led_by_hospital_results.RData')}
...
for_table = mutate(all_diffs_care_review, cell = paste(Event, ' (', CI, ')', sep='')) %>%
  ...
  select(Phase, TPCH, RBWH, GCUH) # in intervention order
#
...
base = basehaz(cox_model_care_review, centered=TRUE) %>%
  ...
  mutate(hosp_ordered = factor(hospital, levels=c('TPCH','RBWH','GCUH')))
...
There is a small non-proportionality for CriSTAL in RBWH with a possibly weaker influence of CriSTAL at the start of the admission. 
...
#
grid.arrange(influence_cox_care_review[['TPCH']],
 influence_cox_care_review[['RBWH']],
 influence_cox_care_review[['GCUH']], ncol=1)
## pick out relatively influential patients
#f = filter(influence_cox_care_review[['GCUH']]$data, covariate=='CriSTAL score')
#index = which(f$res< -0.004)
#View(filter(for_model_care_review, hospital=='GCUH')[index,]) # look at patient's data
```

The x-axis of observation ID is the order in which patients were recruited over time. 

The largest negative outlier for CriSTAL score at TPCH was a for a high score of 10 for a patient with a relatively long time of 6 days until their clinical-led discussion. The relatively large negative outlier for the CriSTAL score at RBWH was for a patient with the largest CriSTAL score of 10 who was discharged before any clinical-led discussion. The relatively large negative outlier for the CriSTAL score at GCUH was for a patient with the largest CriSTAL score of 11 who was discharged before any clinical-led discussion.
...
for_model_care_directive[[k]] = filter(survival_data_imputed[[k]],
                  ...
                  int_time != 'Establishment') %>% # excluded from analysis
  mutate(event = as.character(event), # needed for ifelse below
         event = ifelse(event== 'Prior', 'Yes', event), # prior counted as early event
         event = ifelse(event== 'No', 'Censored', event)) # change "No" to "censored" for this analysis. Competing risks are death/event
...
to_plot_care_directive = mutate(plot_data,
                 ...
                 group = ifelse(group=='Usual', 'Usual care', group))
# redo plot for greater control
alt_plot = ggplot(to_plot_care_directive, aes(x=time, y=est, ymin=lower, ymax=upper, col=group)) +
  geom_ribbon(alpha=0.2) +
  geom_line(size = 1.05) +  # make lines thicker
...
for (k in 1:10){ # loop through imputations
  ...
  for_model_care_directive[[k]] = mutate(for_model_care_directive[[k]], 
    event = ifelse(event=='Prior', 'Yes', event))
  cox_model_care_directive[[k]] = coxph(Surv(time = time, event = event== 'Yes') ~ int_time_n + I(age/5) + pt_sex +  cristal_score + spict_score + strata(team), data = for_model_care_directive[[k]])
...
for (k in 1:10){ # loop through imputations
  cox_model_care_directive_hosp[[k]] = coxph(Surv(time = time, event = event== 'Yes') ~ int_time_n + I(age/5) + pt_sex +  cristal_score + spict_score + strata(team), data = filter(for_model_care_directive[[k]], hospital==hosp))
  #
...
```{r}  
ft = flextable(all_ests_cox_care_directive) %>%
  theme_box() %>%
  autofit()
ft
```
...
if(exists == FALSE){
  risk_difference_care_directive = risk_diff_crr(
    ...
    B = n_boot,
    pred_time = pred_time)
  save(risk_difference_care_directive, file = 'results/care_directive_absolute_results.RData')
}
...
if(exists == FALSE){
  ...
  save(all_diffs_care_directive, file='results/care_directive_absolute_by_hospital_results.RData')
}
if(exists == TRUE){load('results/care_directive_absolute_by_hospital_results.RData')}
# table
...
```{r, fig.height=8, fig.width=9}
#
grid.arrange(influence_cox_care_directive[['TPCH']],
 influence_cox_care_directive[['RBWH']],
 influence_cox_care_directive[['GCUH']], ncol=1)
## pick out relatively influential patients
# a) age
#f = filter(influence_cox_care_directive[['RBWH']]$data, covariate=='Age (+5 years)')
#index = which(f$res< -0.006)
# b) cristal
#f = filter(influence_cox_care_directive[['RBWH']]$data, covariate=='SPICT score')
#index = which(f$res< -0.01)
#View(filter(for_model_care_review, hospital=='RBWH')[index,]) # look at patient's data
```

The x-axis of observation ID is the order in which patients were recruited over time. 

There is one relatively large negative value for age in RBWH; this was a 97 year old who was discharged after 18 days. There is one relatively large negative value for CRiSTAL in RBWH; this was a patient with the highest CriSTAL score of 10 who was discharged after 3 days.
...
for_model_palliative = filter(survival_data,
                  ...
                  int_time != 'Establishment') %>% # excluded from analysis
  mutate(event = as.character(event), # needed for ifelse below
         event = ifelse(event== 'Prior', 'Yes', event), # prior to early yes
         event = ifelse(event== 'No', 'Censored', event)) # change "No" to "censored" for this analysis. Competing risks are death/event
...
to_plot_palliative = mutate(plot_data,
                 ...
                 group = ifelse(group=='Usual', 'Usual care', group))
# redo plot for greater control
alt_plot = ggplot(to_plot_palliative, aes(x=time, y=est, ymin=lower, ymax=upper, col=group)) +
  geom_ribbon(alpha=0.2) +
  geom_line(size = 1.05) +  # make lines thicker
  ...
  xlab('Days from ward admission') +
  ylab('Cumulative probability') +
  theme(legend.position = c(0.87, 0.15)) + # these numbers are proportional
  facet_wrap(~hosp_ordered)
alt_plot

# save for 2_plot_paper_outcome456.R
save(to_plot_care_review, to_plot_care_directive, to_plot_palliative, file='results/plot_data.RData')
...
```{r}
# add prior events as events at early time
for_model_palliative= mutate(for_model_palliative, 
    event = ifelse(event=='Prior', 'Yes', event))
# Cox model stratified on team within hospital
cox_model_palliative = coxph(Surv(time = time, event = event== 'Yes') ~ int_time_n + I(age/5) + pt_sex +  cristal_score + spict_score + strata(team), data = for_model_palliative)
# number missing:
...
for (hosp in c("GCUH","TPCH",'RBWH')){
  # Cox model stratified on team within hospital
  cox_model_palliative_hosp = coxph(Surv(time = time, event = event== 'Yes') ~ int_time_n + I(age/5) + pt_sex +  cristal_score + spict_score + strata(team), data = filter(for_model_palliative, hospital==hosp))
  #
...
# make table
  all_ests_cox_palliative = filter(all_ests_cox_palliative, !is.na(estimate)) %>%
  mutate(Variable = nice_rename(term), # see 99_functions
    HR = roundz(exp(estimate),2),
    conf.low = roundz(exp(conf.low),2),
    conf.high = roundz(exp(conf.high),2),
  ...
  pivot_wider(values_from=cell, names_from=hospital) %>%
    select(Variable, TPCH, RBWH, GCUH) # In intervention order
ft = flextable(all_ests_cox_palliative) %>%
  theme_box() %>%
  autofit()
ft
```
...
if(exists == FALSE){
  risk_difference_palliative = risk_diff_crr(
    ...
    B = n_boot,
    pred_time = pred_time)
  save(risk_difference_palliative, file = 'results/palliative_absolute_results.RData')
}
...
if(exists == FALSE){
  ...
  save(all_diffs_palliative, file = 'results/palliative_absolute_by_hospital_results.RData')
}
if(exists == TRUE){load('results/palliative_absolute_by_hospital_results.RData')}
# table
...
# save regression models for 2_main_table_outcome456.Rmd
save(all_ests_cox_care_directive,
     all_ests_cox_care_review,
     all_ests_cox_palliative,
     all_diffs_care_directive,
     all_diffs_care_review,
     all_diffs_palliative, file='results/regression_data.RData')
```
...
```{r, fig.height=8, fig.width=9}
#
grid.arrange(influence_cox_palliative[['TPCH']],
 influence_cox_palliative[['RBWH']],
 influence_cox_palliative[['GCUH']], ncol=1)
```
...
# smooth over calendar time using a window
start_time = floor(window/2)
end_time = floor(max(calendar_time_palliative$start) - (window/2))
...
# save calendar plots for 2_plot_paper_outcome456.R
save(smooth_prop_care_review, smooth_prop_care_directive, smooth_prop_palliative, file='results/plot_data.RData')
```
...
all_dates = expand.grid(hospital=c('TPCH','RBWH','GCUH'), admission_date=all_dates)
# Poisson model
...
## Sensitivity analyses #7: proportion of uncensored patients
...
The table shows the number of patients and percent uncensored, ideally this would be close to 100%. There was a relatively large amount of censoring at TPCH, in particular for palliative care referral, because many patients could not be followed until discharge. 
