...
```{r}
ages = group_by(baseline, at_risk) %>%
...
All results from here on are in at risk patients.


### Patient gender

```{r, results='asis'}
freq(at_risk$pt_sex, cumul=FALSE, report.nas = FALSE)
```
...
to.corr = select(at_risk, spict.vars) %>% # these are just yes/no/unknown
...
spict = select(at_risk, "participant_id", all_of(spict.vars)) %>%
...
to.bar = group_by(at_risk, spict_score) %>%
  summarise(count = n()) %>%
...
with(at_risk, freq(`SPICT positive`, round.digits=0, cumul = FALSE, report.nas =FALSE))
```
...
yes_responses <-  dplyr::select(at_risk, participant_id, spict.vars)  %>%
  as_tibble() %>%
  tidyr::gather(key='spict', value='response', -participant_id) %>%
...
# add zeros
all_ids = unique(at_risk$participant_id)
...
And 8 clinical criteria:
  ...
  * Decreased level of consciousness (LOC) Glasgow Coma Scale (GCS) decreased more than 2 compared with at_risk for this person
...
# estimate correlation matrix
to.corr = select(at_risk, cristal.vars) %>%
...
## big summary table
# create summary stats for table / plot
cristal = select(at_risk, "participant_id", 'hospital', all_of(cristal.vars)) %>%
  mutate(
...
```{r, fig.width=8}
#
MaxC = max(at_risk$cristal_score) # for x-axis limit
#
to.bar = group_by(at_risk, cristal_score) %>%
  summarise(count = n()) %>%
...
descr(at_risk$cristal_score, stats = c('N.valid','mean','sd','min','med','max'), round.digits =1, transpose = TRUE)
```
...
with(at_risk, freq(`CriSTAL positive`, round.digits=0, cumul = FALSE, report.nas =FALSE))
```
...
```{r, fig.width=8}
# get combinations of yes responses
yes_responses <-  dplyr::select(at_risk, participant_id, cristal.vars)  %>%
  mutate(
    #age = ifelse(age>=75, 'Yes', 'No'), # convert to binary - no longer using age in cristal
    cristal_cfs_score1 = ifelse(cristal_cfs_score >= thresholds$frailty_threshold, 'Yes', 'No'), # frailty score over 5
    cristal_cfs_score2 = ifelse(cristal_cfs_score >= thresholds$frailty_threshold2, 'Yes', 'No')) %>%  # frailty score over 7
...
```{r, out.width='100%', fig.width=7}
p <- ggplot(filter(at_risk, !is.na(at_risk)), aes(x=factor(at_risk), y=cristal_cfs_score, fill=factor(at_risk))) + 
  scale_fill_manual('', values=cbPalette)+
  geom_violin(alpha=0.6, bw=2)+
  geom_boxplot(width=0.1)+
  xlab('')+
