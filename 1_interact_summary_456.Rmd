...
table = group_by(for_table, int_time, hospital, number, name) %>%
  ...
  select('hospital','number','name','Usual care','Establishment','Intervention','Screening not complete') %>% # column ordering
...
for (hosp in c("GCUH","TPCH",'RBWH')){
  ...
  all_ests = filter(all_ests, !is.na(estimate)) %>%
...
##### Absolute measure of difference by hospital
...
for (hosp in c("GCUH","TPCH",'RBWH')){
  risk_difference = risk_diff(
    ...
    pred_time = pred_time) %>%
    mutate(hospital = hosp)
  all_diffs = bind_rows(all_diffs, risk_difference)
}
#
for_table = mutate(all_diffs, cell = paste(Event, ' (', CI, ')', sep='')) %>%
...
The plot below shows the 'dfbetas' which are the estimated changes in the regression coefficients after deleting each observation patient in turn. These plots are for model checking and will not be presented in papers or reports. 
...
The plots below shows the top ten combinations split by intervention phase for all hospitals combined.
...
```{r}
# Cox model stratified on team within hospital
cox_model_care_directive = coxph(Surv(time = time, event = event== 'Yes') ~ int_time_n + I(age/5) + pt_sex +  cristal_score + spict_score + strata(team), data = for_model_care_directive)
# number missing:
...
for (hosp in c("GCUH","TPCH",'RBWH')){
  # Cox model stratified on team within hospital
  cox_model_care_directive_hosp = coxph(Surv(time = time, event = event== 'Yes') ~ int_time_n + I(age/5) + pt_sex +  cristal_score + spict_score + strata(team), data = filter(for_model_care_directive, hospital==hosp))
  #
  ests = tidy(cox_model_care_directive_hosp, conf.int = TRUE) %>%
    mutate(hospital = hosp)
  all_ests = bind_rows(all_ests, ests)
}
  all_ests = filter(all_ests, !is.na(estimate)) %>%
...
for (hosp in c("GCUH","TPCH",'RBWH')){
  risk_difference = risk_diff(
    indata = filter(for_model_care_directive, hospital==hosp),
    inmodel= cox_model_care_directive,
    B = n_boot,
    pred_time = pred_time) %>%
    mutate(hospital = hosp)
...
#### Plot of baseline cumulative hazard for clinician-led care review discussion
...
# plot
bplot = ggplot(data=diff_plot, aes(x=time, y=diff, col=hosp_ordered)) +
...
The plot shows the difference in the baseline cumulative hazard from a simple linear interpolation of the hazard between the first and last time. Higher values indicate more events than average, and lower values fewer events. The vertical dotted line shows the change-over to the establishment phase. 

The four weeks of establishment phase have been removed to avoid a break in the plot.
...
#### Plot of baseline cumulative hazard for care directives documents
...
#### Plot of baseline cumulative hazard for palliative care referral
...
less_baseline = select(baseline, -age, -pt_sex, -team, -cristal_score, -spict_score)
...
Here we examine unplanned admissions over time. We exclude general medical wards. The aim is to see if COVID had an impact on the types of patients over time. 

```{r, fig.width=8}
for_plot = filter(baseline, 
                   redcap_version == 3,
                   at_risk == "At risk",
                  !str_detect(string=team, pattern='Medicine')) %>% # exclude gen-med
...
The smooth line is applied to a numerical version of unplanned hospital admission that is 1 for "yes" and otherwise. The confidence interval for the smooth is sometimes greater than 1. 
